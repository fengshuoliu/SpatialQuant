...
enum OverlayRenderer {
    ...
    static func render(
        ...
        guard let firstChannel = overlayChannels.first,
            ...
            throw SpatialPlexomeraError.message("No CSV channels are selected for overlay rendering.")
        }

        for channel in overlayChannels {
            guard let matrix = matrixByFile[channel.fileName] else {
                throw SpatialPlexomeraError.message("Missing matrix for \(channel.fileName).")
            }
            if matrix.width != firstMatrix.width || matrix.height != firstMatrix.height {
                throw SpatialPlexomeraError.message("\(channel.fileName) has shape \(matrix.width)x\(matrix.height), expected \(firstMatrix.width)x\(firstMatrix.height).")
            }
        }
    ...
    private static func renderSplitPanels(
        ...
        guard !channelMatrices.isEmpty else {
            throw SpatialPlexomeraError.message("No matrices are loaded.")
        }
