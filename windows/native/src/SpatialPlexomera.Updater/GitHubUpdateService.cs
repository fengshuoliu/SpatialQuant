...
{
    public const string InstallerAssetName = "SpatialPlexomera-Windows-x64-Setup.exe";
    public const string ChecksumAssetName = "SHA256SUMS-Windows.txt";
    public static readonly Uri ReleasesApiUri = new(
        "https://api.github.com/repos/fengshuoliu/SpatialPlexomera/releases?per_page=100");
    ...
    public async Task<WindowsUpdateRelease?> CheckForUpdateAsync(CancellationToken cancellationToken = default)
    {
        ...
        request.Headers.UserAgent.ParseAdd($"SpatialPlexomera-Windows/{FormatVersion(_currentVersion)}");
        ...
        out WindowsUpdateRelease candidate)
    {
        ...
        var releasePage = new Uri(
            $"https://github.com/fengshuoliu/SpatialPlexomera/releases/tag/{Uri.EscapeDataString(tagName)}");
    ...
    {
        var expectedPath = $"/fengshuoliu/SpatialPlexomera/releases/download/{Uri.EscapeDataString(tagName)}/{Uri.EscapeDataString(assetName)}";
