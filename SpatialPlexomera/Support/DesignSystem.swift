...
struct SpatialPlexomeraCanvasBackground: View {
    @Environment(\.colorScheme) private var colorScheme

    var body: some View {
        ZStack {
            SpatialPlexomeraDesign.canvas

            LinearGradient(
                stops: [
                    .init(color: SpatialPlexomeraDesign.spectralCyan.opacity(spectralOpacity), location: 0),
                    .init(color: Color.clear, location: 0.34),
                    .init(color: SpatialPlexomeraDesign.spectralMagenta.opacity(spectralOpacity * 0.52), location: 0.69),
                    .init(color: SpatialPlexomeraDesign.spectralGreen.opacity(spectralOpacity * 0.76), location: 1)
                ],
...
extension View {
    func spatialPlexomeraWindowMaterial() -> some View {
        modifier(SpatialPlexomeraWindowMaterialModifier())
    }

    func spatialPlexomeraGlassSurface(
        cornerRadius: CGFloat = SpatialPlexomeraDesign.panelRadius,
        tint: Color? = nil,
        interactive: Bool = false
    ) -> some View {
        modifier(
            SpatialPlexomeraGlassSurfaceModifier(
    ...
    func spatialPlexomeraGlassCapsule(tint: Color? = nil) -> some View {
        modifier(SpatialPlexomeraGlassCapsuleModifier(tint: tint))
    }

    func spatialPlexomeraSelectedGlass(isSelected: Bool, tint: Color) -> some View {
        modifier(SpatialPlexomeraSelectedGlassModifier(isSelected: isSelected, tint: tint))
    }

    func spatialPlexomeraProminentButtonStyle() -> some View {
        modifier(SpatialPlexomeraProminentButtonModifier())
    }
}

private struct SpatialPlexomeraWindowMaterialModifier: ViewModifier {
    @ViewBuilder
...
private struct SpatialPlexomeraGlassSurfaceModifier: ViewModifier {
    ...
    func body(content: Content) -> some View {
        ...
        if reduceTransparency {
            content
                .background(SpatialPlexomeraDesign.panel, in: shape)
                .overlay {
                    shape.stroke(SpatialPlexomeraDesign.panelBorder, lineWidth: 1)
                }
                .shadow(color: SpatialPlexomeraDesign.glassAmbientShadow, radius: 10, y: 4)
        } else if #available(macOS 26.0, *) {
            if interactive {
                content
                    .glassEffect(.regular.tint(tint ?? SpatialPlexomeraDesign.glassNeutralTint).interactive(), in: shape)
                    .overlay {
                        SpatialPlexomeraOpticalEdge(shape: shape, tint: tint, intensity: isHovering ? 1.12 : 0.92)
                    }
                    .shadow(color: SpatialPlexomeraDesign.glassShadow, radius: isHovering ? 18 : 14, y: isHovering ? 7 : 5)
                    .shadow(color: SpatialPlexomeraDesign.glassAmbientShadow, radius: 4, y: -1)
                ...
                content
                    .glassEffect(.regular.tint(tint ?? SpatialPlexomeraDesign.glassNeutralTint), in: shape)
                    .overlay {
                        SpatialPlexomeraOpticalEdge(shape: shape, tint: tint, intensity: 0.82)
                    }
                    .shadow(color: SpatialPlexomeraDesign.glassShadow, radius: 15, y: 6)
                    .shadow(color: SpatialPlexomeraDesign.glassAmbientShadow, radius: 4, y: -1)
            }
        } else {
            content
...
private struct SpatialPlexomeraGlassCapsuleModifier: ViewModifier {
    var tint: Color?
    @Environment(\.accessibilityReduceTransparency) private var reduceTransparency
    ...
    func body(content: Content) -> some View {
        if reduceTransparency {
            content
                .background(SpatialPlexomeraDesign.panel, in: Capsule())
                .overlay {
                    Capsule().stroke(SpatialPlexomeraDesign.panelBorder, lineWidth: 1)
                }
        } else if #available(macOS 26.0, *) {
            content
                .glassEffect(.regular.tint(tint ?? SpatialPlexomeraDesign.glassNeutralTint), in: Capsule())
                .overlay {
                    SpatialPlexomeraOpticalEdge(shape: Capsule(), tint: tint, intensity: 0.88)
                }
                .shadow(color: SpatialPlexomeraDesign.glassAmbientShadow, radius: 7, y: 3)
        } else {
            content
...
private struct SpatialPlexomeraSelectedGlassModifier: ViewModifier {
    var isSelected: Bool
    var tint: Color
    ...
    func body(content: Content) -> some View {
        if #available(macOS 26.0, *), isSelected {
            ...
            content
                ...
                .overlay {
                    SpatialPlexomeraOpticalEdge(shape: shape, tint: tint, intensity: 1.1)
                }
...
private struct SpatialPlexomeraProminentButtonModifier: ViewModifier {
    @ViewBuilder
    func body(content: Content) -> some View {
...
private struct SpatialPlexomeraOpticalEdge<S: InsettableShape>: View {
    var shape: S
    var tint: Color?
            ...
            shape.stroke(
                LinearGradient(
                    stops: [
                        ...
                        .init(color: SpatialPlexomeraDesign.spectralCyan.opacity(0.40 * intensity), location: 0.18),
                        .init(color: Color.white.opacity(0.14 * intensity), location: 0.45),
                        .init(color: semanticTint.opacity(0.34 * intensity), location: 0.67),
                        .init(color: SpatialPlexomeraDesign.spectralMagenta.opacity(0.22 * intensity), location: 0.82),
...
struct WorkflowStatusBadge: View {
    ...
    var body: some View {
        Text(LocalizedStringKey(status.title))
            ...
            .padding(.vertical, 5)
            .spatialPlexomeraGlassCapsule(tint: status.foregroundColor.opacity(0.16))
