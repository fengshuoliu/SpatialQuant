...
namespace SpatialQuant.Windows.Updates;

public static class UpdateInstallerLauncher
{
    public const string InstanceMutexName = @"Local\SpatialQuant.Windows.Application";
    public const string QaInstanceMutexEnvironmentVariable = "SPATIALQUANT_QA_INSTANCE_MUTEX";
    public const string QaSmokeLaunchArgument = "--qa-smoke";
    public const string QaSmokeMarkerFileName = ".spatialquant-smoke";
    private const string QaInstanceMutexPrefix = @"Local\SpatialQuant.Windows.Application.QA.";
        ...
        int parentProcessId)
    {
        ...
        return Process.Start(startInfo)
            ?? throw new InvalidOperationException("Windows did not start the SpatialQuant update installer.");
    }
    ...
    private static void EnsureChildPath(string candidate, string expectedParent)
    {
        ...
        {
            throw new InvalidOperationException("The installer path escaped the private SpatialQuant update directory.");
        }
    }
}
