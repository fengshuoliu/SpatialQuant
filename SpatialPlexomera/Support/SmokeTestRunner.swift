...
enum SmokeTestRunner {
    ...
    private static func smokeInputFolder() -> URL {
        smokeFolder(environmentKey: "SPATIALPLEXOMERA_SMOKE_INPUT", defaultName: "test_images_input_files")
    }

    private static func smokeOutputFolder() -> URL {
        smokeFolder(environmentKey: "SPATIALPLEXOMERA_SMOKE_OUTPUT", defaultName: "test_images_output_files")
    }
    ...
    static func refreshCellTypeMaskFromSavedAssignment() throws {
        ...
        guard let result = OutputWriter.loadCellTypeAssignmentResult(outputFolder: outputFolder) else {
            throw SpatialPlexomeraError.message("Could not load saved cell-type assignment outputs.")
        }
    ...
    static func runNucleiSmokeTest(
        ...
        guard let nucleusURL = csvURLs.first(where: { $0.lastPathComponent.lowercased().contains("ir193") })
            ...
            ?? csvURLs.first else {
            throw SpatialPlexomeraError.message("No CSV file available for nuclei smoke test.")
        }
        ...
        if runScan {
            ...
            guard let best = records.max(by: { $0.count < $1.count }) else {
                throw SpatialPlexomeraError.message("Nuclei scan produced no records.")
            }
            try OutputWriter.writeConfiguration(
                inputFolder: inputFolder,
                outputFolder: outputFolder,
    ...
    static func runCellTypeAssignmentSmokeTest(cpuAllocationPercent: Double = 100) throws {
        ...
        guard !detections.isEmpty else {
            throw SpatialPlexomeraError.message("Cell-type smoke test could not find nuclei detections.")
        }
    ...
    static func runCellTypeAssignmentScreeningSmokeTest(
        ...
        guard !detections.isEmpty else {
            throw SpatialPlexomeraError.message("Cell-type screening smoke test could not find nuclei detections.")
        }
    ...
    static func runNeighborhoodSmokeTest(cpuAllocationPercent: Double = 100) throws {
        ...
        guard let assignmentResult else {
            throw SpatialPlexomeraError.message("Neighborhood smoke test could not load cell-type assignment output.")
        }
    ...
    static func runRegionSmokeTest(cpuAllocationPercent: Double = 100) throws {
        ...
        guard let assignmentResult else {
            throw SpatialPlexomeraError.message("Region smoke test could not load cell-type assignment output.")
        }
        ...
        guard !assignedCellTypes.isEmpty else {
            throw SpatialPlexomeraError.message("Region smoke test found no assigned cell type.")
        }
    ...
    private static func validateRegionGeometry(_ result: RegionAnalysisResult) throws {
        guard !result.regions.isEmpty else {
            throw SpatialPlexomeraError.message("Region regression check found no ROI geometry.")
        }
        ...
        guard hasOrganicTwoDimensionalBoundary else {
            throw SpatialPlexomeraError.message("Region regression check detected flattened or scanline-dominated ROI boundaries.")
        }
    }

    private static func regionSmokeOutputFolder() -> URL {
        let override = ProcessInfo.processInfo.environment["SPATIALPLEXOMERA_REGION_SMOKE_OUTPUT"]?
            .trimmingCharacters(in: .whitespacesAndNewlines)
    ...
    static func runRegionCustomizedDisplaySmokeTest(cpuAllocationPercent: Double = 100) throws {
        ...
        guard let assignmentResult else {
            throw SpatialPlexomeraError.message("Region customized display smoke test could not load cell-type assignment output.")
        }
        ...
        guard let regionResult else {
            throw SpatialPlexomeraError.message("Region customized display smoke test could not load region analysis output.")
        }
        guard let firstRegionID = regionResult.regions.sorted(by: { $0.id < $1.id }).first?.id else {
            throw SpatialPlexomeraError.message("Region customized display smoke test found no region boundaries.")
        }
        ...
        guard let firstCellType else {
            throw SpatialPlexomeraError.message("Region customized display smoke test found no assigned cell type.")
        }
    ...
    static func runRegionManualAdjustmentSmokeTest(cpuAllocationPercent: Double = 100) throws {
        ...
        guard let assignmentResult else {
            throw SpatialPlexomeraError.message("Region manual adjustment smoke test could not load cell-type assignment output.")
        }
        ...
        guard let baseRegionResult = regionResult else {
            throw SpatialPlexomeraError.message("Region manual adjustment smoke test could not load region analysis output.")
        }
        guard let targetRegion = baseRegionResult.regions.sorted(by: { $0.id < $1.id }).first else {
            throw SpatialPlexomeraError.message("Region manual adjustment smoke test found no base ROI.")
        }
        ...
        guard let loaded = OutputWriter.loadRegionAnalysisResult(outputFolder: outputFolder) else {
            throw SpatialPlexomeraError.message("Region manual adjustment smoke test could not reload adjusted region output.")
        }
        let loadedRegionIDs = Set(loaded.regions.map(\.id))
        guard baseRegionIDs.isSubset(of: loadedRegionIDs) else {
            throw SpatialPlexomeraError.message("Manual ROI save dropped one or more pre-existing region boundaries.")
        }
        ...
        for expected in ["manual_drawn_smoke_ROI", "adjusted_include_smoke_ROI", "adjusted_exclude_smoke_ROI"] {
            guard sourceTypes.contains(expected) else {
                throw SpatialPlexomeraError.message("Adjusted ROI \(expected) was not present after reload.")
            }
        }
        ...
        guard registryText.contains("manual_region_adjustment") else {
            throw SpatialPlexomeraError.message("Boundary mask registry did not record manual_region_adjustment entries.")
        }

        guard loaded.regions.count == adjustedResult.regions.count,
              loaded.regions.count > adjustedRenderRegions.count else {
            throw SpatialPlexomeraError.message("Manual ROI export did not preserve base boundaries plus adjusted boundaries.")
        }
    ...
    static func runCellDistributionSmokeTest(cpuAllocationPercent: Double = 100) throws {
        ...
        guard let assignmentResult else {
            throw SpatialPlexomeraError.message("Cell distribution smoke test could not load cell-type assignment output.")
        }
        ...
        guard let regionResult else {
            throw SpatialPlexomeraError.message("Cell distribution smoke test could not load region analysis output.")
        }
        let smokeRegions = regionResult.regions.sorted(by: { $0.id < $1.id })
        guard let firstRegion = smokeRegions.first else {
            throw SpatialPlexomeraError.message("Cell distribution smoke test found no region boundaries.")
        }
        ...
        let assignedCellTypes = Set(assignmentResult.assignments
            ...
            .sorted { $0.localizedStandardCompare($1) == .orderedAscending }
        guard !assignedCellTypes.isEmpty else {
            throw SpatialPlexomeraError.message("Cell distribution smoke test found no assigned cell type for density.")
        }
        try OutputWriter.runStreamlitCellDistributionExport(
            outputFolder: outputFolder,
            mode: .cellDensity,
        ...
        guard let densityResult = OutputWriter.loadCellDistributionResult(outputFolder: outputFolder),
            ...
            throw SpatialPlexomeraError.message("Cell distribution smoke test did not reload Streamlit density outputs.")
        }
        guard densityResult.regionSummaries.contains(where: { $0.dominantType.contains(latestBoundaryLabel) }),
            ...
            throw SpatialPlexomeraError.message("Cell distribution smoke test did not load the latest boundary with all default cell types.")
        }
    ...
    static func runCellDistributionClusterSmokeTest(cpuAllocationPercent: Double = 100) throws {
        ...
        guard assignmentResult != nil else {
            throw SpatialPlexomeraError.message("Cell distribution cluster smoke test could not load cell-type assignment output.")
        }
        ...
        guard let regionResult else {
            throw SpatialPlexomeraError.message("Cell distribution cluster smoke test could not load region analysis output.")
        }
        ...
        guard let neighborhoodResult else {
            throw SpatialPlexomeraError.message("Cell distribution cluster smoke test could not load neighborhood analysis output.")
        }
        guard let firstRegion = regionResult.regions.sorted(by: { $0.id < $1.id }).first else {
            throw SpatialPlexomeraError.message("Cell distribution cluster smoke test found no region boundaries.")
        }
        try ensureSmokePixelSizeConfiguration(outputFolder: outputFolder, cpuAllocationPercent: cpuAllocationPercent)
        let boundaryLabel = firstRegion.sourceType ?? firstRegion.dominantType
        ...
        guard !clusterLabels.isEmpty else {
            throw SpatialPlexomeraError.message("Cell distribution cluster smoke test found no neighborhood cluster labels.")
        }
        try OutputWriter.runStreamlitCellDistributionExport(
            outputFolder: outputFolder,
        ...
        guard let result = OutputWriter.loadCellDistributionResult(outputFolder: outputFolder),
            ...
            throw SpatialPlexomeraError.message("Cell distribution cluster smoke test did not reload Streamlit cluster outputs.")
        }
    ...
    static func runCellDistributionLoadSmokeTest(cpuAllocationPercent: Double = 100) throws {
        ...
        guard var result else {
            throw SpatialPlexomeraError.message("Cell distribution load smoke test could not reload Streamlit-style outputs.")
        }
        guard !result.bandMetrics.isEmpty else {
            throw SpatialPlexomeraError.message("Cell distribution load smoke test reloaded no density band metrics.")
        }
        if result.clusterMetrics.isEmpty || result.tileClassifications.isEmpty {
            ...
            guard let reloaded = OutputWriter.loadCellDistributionResult(outputFolder: outputFolder) else {
                throw SpatialPlexomeraError.message("Cell distribution load smoke test could not reload cluster distribution outputs.")
            }
            result = reloaded
        }
        guard !result.clusterMetrics.isEmpty, !result.tileClassifications.isEmpty else {
            throw SpatialPlexomeraError.message("Cell distribution load smoke test reloaded no cluster metrics or tile rows.")
        }
    ...
    static func runDistanceSmokeTest(cpuAllocationPercent: Double = 100) throws {
        ...
        guard let assignmentResult else {
            throw SpatialPlexomeraError.message("Distance smoke test could not load cell-type assignment output.")
        }
        ...
        guard var resolvedRegionResult = regionResult else {
            throw SpatialPlexomeraError.message("Distance smoke test could not load region analysis output.")
        }
        ...
        guard let targetType = cellTypes.first else {
            throw SpatialPlexomeraError.message("Distance smoke test found no assigned cell types.")
        }
        ...
        guard let boundaryChoice = boundaryChoices.first else {
            throw SpatialPlexomeraError.message("Distance smoke test found no saved boundary masks.")
        }
        ...
        guard let loaded = OutputWriter.loadDistanceAnalysisResult(outputFolder: outputFolder),
            ...
            throw SpatialPlexomeraError.message("Distance smoke test could not reload both nearest-neighbor and boundary outputs.")
        }
