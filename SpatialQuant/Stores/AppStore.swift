...
final class AppStore: ObservableObject {
    ...
    private let logger = Logger(subsystem: "com.fengshuoliu.SpatialQuant", category: "Pipeline")
    private let defaults = UserDefaults.standard
    private let inputFolderKey = "SpatialQuant.inputFolder"
    private let outputFolderKey = "SpatialQuant.outputFolder"
    private let cpuAllocationKey = "SpatialQuant.cpuAllocationPercent"
    private let gpuAllocationKey = "SpatialQuant.gpuAllocationPercent"
    private let maximumCPUDefaultMigrationKey = "SpatialQuant4.maximumCPUDefaultApplied"
    private let nucleiScanCombinationBudgetKey = "SpatialQuant.nucleiScanCombinationBudget"
    private let nucleiScanSecondsPerCombinationKey = "SpatialQuant.nucleiScanSecondsPerCombination"
    private let nucleiScanBenchmarkCPUAllocationKey = "SpatialQuant.nucleiScanBenchmarkCPUAllocation"
    ...
    private static func defaultWorkspaceFolder(named name: String) -> URL {
        ...
        return documents
            .appendingPathComponent("SpatialQuant", isDirectory: true)
        ...
        currentTask = Task.detached(priority: .userInitiated) { [store = self] in
            do {
                ...
                guard !adjustedRegionIDs.isEmpty else {
                    throw SpatialQuantError.message("No adjusted ROI was generated from the selected cells.")
                }
        ...
        currentTask = Task.detached(priority: .userInitiated) { [store = self] in
            do {
                ...
                guard let displayResult = OutputWriter.loadCellDistributionResult(outputFolder: outputFolder) else {
                    throw SpatialQuantError.message("Cell Distribution export finished but no readable result was found.")
                }
                await MainActor.run {
