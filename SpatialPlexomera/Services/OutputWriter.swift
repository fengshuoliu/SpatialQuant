...
enum OutputWriter {
    ...
    static func writeCustomizedRegionDisplayOutputs(
        ...
        guard !resolvedRegionIDs.isEmpty else {
            throw SpatialPlexomeraError.message("Select at least one region boundary to save.")
        }
        guard !resolvedCellTypes.isEmpty else {
            throw SpatialPlexomeraError.message("Select at least one cell type to display.")
        }
        ...
        guard !selectedRegions.isEmpty else {
            throw SpatialPlexomeraError.message("No selected boundary matches the selected cell type.")
        }
    ...
    private static func streamlitPythonExecutableURL() -> URL? {
        ...
        if let configuredPath = ProcessInfo.processInfo.environment["SPATIALPLEXOMERA_PYTHON"],
    ...
    private static func runStreamlitCellDistributionExporterIfAvailable(
        ...
        guard process.terminationStatus == 0 else {
            ...
            let details = [stderr, stdout]
                ...
                .filter { !$0.isEmpty }
                .joined(separator: "\n")
            throw SpatialPlexomeraError.message("Cell Distribution export failed: \(details)")
        }
    }
    ...
    static func runStreamlitCellDistributionExport(
        ...
        guard hasBundledRuntime || hasDevelopmentRuntime else {
            throw SpatialPlexomeraError.message("The Cell Distribution runtime is missing from this app build.")
        }
    ...
    static func loadLatestCellDistributionRegionMaskArtifacts(outputFolder: URL) throws -> CellDistributionRegionMaskArtifacts {
        ...
        guard let arraysURL = matchingRegionMaskArtifact(
            ...
            throw SpatialPlexomeraError.message("Generate Region masks before running Cell density.")
        }
        guard let bandMapURL = matchingRegionMaskArtifact(
            ...
            throw SpatialPlexomeraError.message("The saved Region masks band map could not be loaded.")
        }
        ...
        guard insideBandIndex.width == outsideBandIndex.width,
            ...
            throw SpatialPlexomeraError.message("Saved Region masks band arrays have mismatched dimensions.")
        }
