...
namespace SpatialPlexomera.Windows.Services;
...
public sealed class EngineClient : IAsyncDisposable
{
    ...
    public async Task StartAsync(CancellationToken cancellationToken = default)
    {
        ...
        startInfo.Environment["SPATIALPLEXOMERA_CPU_WORKERS"] = cpuCount;
        startInfo.Environment["SPATIALPLEXOMERA_GPU_MODE"] = "auto";
        ...
        _process = Process.Start(startInfo) ?? throw new InvalidOperationException("Could not start the SpatialPlexomera analysis engine.");
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        finally
        {
            ...
            var stopped = new ObjectDisposedException(nameof(EngineClient), "The analysis engine was stopped because SpatialPlexomera is closing.");
    ...
    private static (string Executable, string Arguments, string WorkingDirectory) ResolveLaunchCommand()
    {
        ...
        {
            Path.Combine(baseDirectory, "engine", "SpatialPlexomeraEngine.exe"),
            Path.Combine(baseDirectory, "SpatialPlexomeraEngine.exe"),
        };
        ...
        var configuredEngine = Environment.GetEnvironmentVariable("SPATIALPLEXOMERA_ENGINE_PATH");
        ...
        throw new FileNotFoundException("SpatialPlexomeraEngine.exe or windows/backend/native_engine.py was not found.");
    }
}
