...
struct ContentView: View {
    ...
    private var glassContainedDetailContent: some View {
        if #available(macOS 26.0, *) {
            GlassEffectContainer(spacing: SpatialQuantDesign.sectionSpacing) {
                styledDetailContent
            }
        } else {
            styledDetailContent
    ...
    var body: some View {
        HStack(spacing: 15) {
            Image(systemName: section.systemImage)
                ...
                .spatialQuantGlassSurface(cornerRadius: 8, tint: Color.accentColor.opacity(0.12))
        ...
        .padding(.vertical, 17)
        .spatialQuantGlassSurface(cornerRadius: 0, tint: Color.accentColor.opacity(0.035))
    }
}
