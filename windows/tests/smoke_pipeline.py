...
from src.spatialplexomera_analysis.io import files_to_long_df, write_json, zip_directory_bytes  # noqa: E402
from src.spatialplexomera_analysis.models import ChannelConfig, NucleiParams, PipelineConfig, RegionParams  # noqa: E402
...
from src.spatialplexomera_analysis.visualization import overlay_multi_channels, plot_split_channels  # noqa: E402
...
def run_smoke(output_root: Path) -> dict[str, Any]:
    ...
    output_dir = output_root / "SpatialPlexomera_outputs"
    ...
    required_files = [
        ...
        Path(neighborhood_saved["saved_paths"]["map_png"]),
        Path(neighborhood_saved["saved_paths"]["legend_png"]),
        Path(neighborhood_saved["saved_paths"]["legend_csv"]),
        boundary_mask_path,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run every SpatialPlexomera analysis stage on synthetic data.")
    parser.add_argument(
        "--output-root",
        type=Path,
