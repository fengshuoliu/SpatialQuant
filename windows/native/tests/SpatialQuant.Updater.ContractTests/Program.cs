using SpatialQuant.Windows.Updates;
using System.Net;
...
static async Task ReleaseSelectionIsolatesWindowsChannelAsync()
{
    ...
    var releases = new object[]
    {
        Release("v9.0.0", "2026-08-10T00:00:00Z", 90, false, false,
            Asset("SpatialQuant-macOS-universal.dmg", "v9.0.0", 100, hash)),
        ...
        Release("windows-v2.0.0", "2026-08-07T00:00:00Z", 87, false, false,
            Asset("SpatialQuant-Windows-x64-Portable-2.0.0.zip", "windows-v2.0.0", 100, hash),
...
static async Task ApiRequestUsesRequiredHeadersAsync()
{
    ...
    Assert(captured!.Headers.UserAgent.ToString().StartsWith("SpatialQuant-Windows/1.2.5", StringComparison.Ordinal), "Missing updater User-Agent.");
...
{
    var installerBytes = Encoding.UTF8.GetBytes("verified SpatialQuant installer bytes");
...
static async Task QaInstanceMutexOverrideIsIsolatedAndValidatedAsync()
{
    ...
    await WithTemporaryDirectoryAsync(async root =>
    {
        var isolated = $@"Local\SpatialQuant.Windows.Application.QA.{Guid.NewGuid():N}";
            ...
            foreach (var invalid in new[]
                     {
                         ...
                         @"Local\SpatialQuant.Windows.Application.QA.0123456789abcdef0123456789abcde\",
                         @"Local\SpatialQuant.Windows.Application.QA.0123456789abcdef0123456789abcdef0",
                     })
            {
...
static object Asset(string name, string tag, long size, string? hash, string? url = null) => new
{
    ...
    state = "uploaded",
    size,
    browser_download_url = url ?? $"https://github.com/fengshuoliu/SpatialQuant/releases/download/{tag}/{name}",
...
static WindowsUpdateRelease CreateDownloadRelease(Version version, string hash, long installerSize)
{
    ...
    var root = $"https://github.com/fengshuoliu/SpatialQuant/releases";
...
{
    var root = Path.Combine(Path.GetTempPath(), $"SpatialQuant-updater-tests-{Guid.NewGuid():N}");
