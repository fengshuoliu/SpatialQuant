...
!define APP_NAME "SpatialQuant"
!define APP_PUBLISHER "SpatialQuant"
!define APP_REGISTRY_KEY "Software\Microsoft\Windows\CurrentVersion\Uninstall\SpatialQuant"
!define INSTALL_MARKER ".spatialquant-install"
!define SMOKE_MARKER ".spatialquant-smoke"
!define INSTANCE_MUTEX "Local\SpatialQuant.Windows.Application"
!define QA_INSTANCE_MUTEX_PREFIX "Local\SpatialQuant.Windows.Application.QA."
...
Name "${APP_NAME}"
OutFile "${OUTPUT_DIR}\SpatialQuant-Windows-x64-Setup.exe"
InstallDir "$LOCALAPPDATA\Programs\SpatialQuant"
InstallDirRegKey HKCU "${APP_REGISTRY_KEY}" "InstallLocation"
RequestExecutionLevel user
SetCompressor /SOLID lzma
SetCompressorDictSize 64
BrandingText "SpatialQuant ${APP_VERSION}"
Icon "${ICON_PATH}"
UninstallIcon "${ICON_PATH}"

VIProductVersion "${APP_VERSION}.0"
VIAddVersionKey /LANG=1033 "ProductName" "SpatialQuant for Windows"
VIAddVersionKey /LANG=1033 "ProductVersion" "${APP_VERSION}"
VIAddVersionKey /LANG=1033 "CompanyName" "${APP_PUBLISHER}"
VIAddVersionKey /LANG=1033 "LegalCopyright" "Copyright SpatialQuant contributors"
VIAddVersionKey /LANG=1033 "FileDescription" "SpatialQuant installer"
VIAddVersionKey /LANG=1033 "FileVersion" "${APP_VERSION}.0"

!define MUI_ABORTWARNING
!define MUI_FINISHPAGE_RUN "$INSTDIR\SpatialQuant.exe"
!define MUI_FINISHPAGE_RUN_TEXT "Launch SpatialQuant"
...
Function .onInit
  ...
  ${IfNot} ${Errors}
    StrCpy $SmokeMode "1"
    ReadEnvStr $R2 "SPATIALQUANT_QA_INSTANCE_MUTEX"
    ${If} $R2 != ""
      StrLen $R3 $R2
  ...
  ; The verified installer is started before SpatialQuant closes. Wait for
...
Section "SpatialQuant" SEC_MAIN
  ...
  ; Only replace files in a directory that a previous SpatialQuant installer
  ...
  IfFileExists "$INSTDIR\*.*" 0 fresh_install
    MessageBox MB_OK|MB_ICONSTOP "The selected folder is not empty and is not owned by SpatialQuant. Choose an empty folder or the existing SpatialQuant installation folder." /SD IDOK
    SetErrorLevel 2
    Abort

  owned_install:
    Delete "$INSTDIR\SpatialQuant.exe"
    RMDir /r "$INSTDIR\engine"

  fresh_install:
  SetOutPath "$INSTDIR"
  File "${SOURCE_DIR}\SpatialQuant.exe"
  File /r "${SOURCE_DIR}\engine"

  WriteUninstaller "$INSTDIR\Uninstall SpatialQuant.exe"

  FileOpen $R0 "$INSTDIR\${INSTALL_MARKER}" w
  FileWrite $R0 "SpatialQuant ${APP_VERSION}$\r$\n"
  FileClose $R0
  ...
  ${Else}
    ...
    CreateDirectory "$SMPROGRAMS\SpatialQuant"
    CreateShortcut "$SMPROGRAMS\SpatialQuant\SpatialQuant.lnk" "$INSTDIR\SpatialQuant.exe" "" "$INSTDIR\SpatialQuant.exe"
    CreateShortcut "$SMPROGRAMS\SpatialQuant\Uninstall SpatialQuant.lnk" "$INSTDIR\Uninstall SpatialQuant.exe"
    CreateShortcut "$DESKTOP\SpatialQuant.lnk" "$INSTDIR\SpatialQuant.exe" "" "$INSTDIR\SpatialQuant.exe"

    WriteRegStr HKCU "${APP_REGISTRY_KEY}" "DisplayName" "SpatialQuant"
    ...
    WriteRegStr HKCU "${APP_REGISTRY_KEY}" "DisplayIcon" "$INSTDIR\SpatialQuant.exe"
    WriteRegStr HKCU "${APP_REGISTRY_KEY}" "UninstallString" "$\"$INSTDIR\Uninstall SpatialQuant.exe$\""
  ...
  ${If} $UpdateMode == "1"
    ...
    ${EndIf}
    ${If} $SmokeMode == "1"
      Exec '"$INSTDIR\SpatialQuant.exe" ${QA_SMOKE_ARGUMENT}'
    ${Else}
      Exec '"$INSTDIR\SpatialQuant.exe"'
    ${EndIf}
  ${EndIf}
SectionEnd
...
Section "Uninstall"
  SetShellVarContext current
  ${If} $SmokeMode != "1"
    Delete "$DESKTOP\SpatialQuant.lnk"
    Delete "$SMPROGRAMS\SpatialQuant\SpatialQuant.lnk"
    Delete "$SMPROGRAMS\SpatialQuant\Uninstall SpatialQuant.lnk"
    RMDir "$SMPROGRAMS\SpatialQuant"
  ...
  Delete "$INSTDIR\Uninstall SpatialQuant.exe"
  RMDir "$INSTDIR"
SectionEnd
