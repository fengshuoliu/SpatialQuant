...
enum CellTypeAssigner {
    ...
    static func run(
        ...
        guard !detections.isEmpty else {
            throw SpatialPlexomeraError.message("Run final nuclei segmentation before cell-type assignment.")
        }
        guard let first = matrices.first else {
            throw SpatialPlexomeraError.message("No channel matrices are loaded for cell-type assignment.")
        }
        for matrix in matrices where matrix.width != first.width || matrix.height != first.height {
            throw SpatialPlexomeraError.message("\(matrix.fileName) has shape \(matrix.width)x\(matrix.height), expected \(first.width)x\(first.height).")
        }
        ...
        guard !profiles.isEmpty else {
            throw SpatialPlexomeraError.message("No marker channels are available for cell-type assignment.")
        }
    ...
    static func runParameterScreening(
        ...
        guard !detections.isEmpty else {
            throw SpatialPlexomeraError.message("Run final nuclei segmentation before assignment screening.")
        }
        guard let first = matrices.first else {
            throw SpatialPlexomeraError.message("No channel matrices are loaded for assignment screening.")
        }
        for matrix in matrices where matrix.width != first.width || matrix.height != first.height {
            throw SpatialPlexomeraError.message("\(matrix.fileName) has shape \(matrix.width)x\(matrix.height), expected \(first.width)x\(first.height).")
        }
