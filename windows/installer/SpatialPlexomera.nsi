...
!define APP_NAME "SpatialPlexomera"
!define APP_PUBLISHER "SpatialPlexomera"
!define APP_REGISTRY_KEY "Software\Microsoft\Windows\CurrentVersion\Uninstall\SpatialPlexomera"
!define INSTALL_MARKER ".spatialplexomera-install"
!define SMOKE_MARKER ".spatialplexomera-smoke"
!define INSTANCE_MUTEX "Local\SpatialPlexomera.Windows.Application"
!define QA_INSTANCE_MUTEX_PREFIX "Local\SpatialPlexomera.Windows.Application.QA."
...
Name "${APP_NAME}"
OutFile "${OUTPUT_DIR}\SpatialPlexomera-Windows-x64-Setup.exe"
InstallDir "$LOCALAPPDATA\Programs\SpatialPlexomera"
InstallDirRegKey HKCU "${APP_REGISTRY_KEY}" "InstallLocation"
RequestExecutionLevel user
SetCompressor /SOLID lzma
SetCompressorDictSize 64
BrandingText "SpatialPlexomera ${APP_VERSION}"
Icon "${ICON_PATH}"
UninstallIcon "${ICON_PATH}"

VIProductVersion "${APP_VERSION}.0"
VIAddVersionKey /LANG=1033 "ProductName" "SpatialPlexomera for Windows"
VIAddVersionKey /LANG=1033 "ProductVersion" "${APP_VERSION}"
VIAddVersionKey /LANG=1033 "CompanyName" "${APP_PUBLISHER}"
VIAddVersionKey /LANG=1033 "LegalCopyright" "Copyright SpatialPlexomera contributors"
VIAddVersionKey /LANG=1033 "FileDescription" "SpatialPlexomera installer"
VIAddVersionKey /LANG=1033 "FileVersion" "${APP_VERSION}.0"

!define MUI_ABORTWARNING
!define MUI_FINISHPAGE_RUN "$INSTDIR\SpatialPlexomera.exe"
!define MUI_FINISHPAGE_RUN_TEXT "Launch SpatialPlexomera"
...
Function .onInit
  ...
  ${IfNot} ${Errors}
    StrCpy $SmokeMode "1"
    ReadEnvStr $R2 "SPATIALPLEXOMERA_QA_INSTANCE_MUTEX"
    ${If} $R2 != ""
      StrLen $R3 $R2
  ...
  ; The verified installer is started before SpatialPlexomera closes. Wait for
...
Section "SpatialPlexomera" SEC_MAIN
  ...
  ; Only replace files in a directory that a previous SpatialPlexomera installer
  ...
  IfFileExists "$INSTDIR\*.*" 0 fresh_install
    MessageBox MB_OK|MB_ICONSTOP "The selected folder is not empty and is not owned by SpatialPlexomera. Choose an empty folder or the existing SpatialPlexomera installation folder." /SD IDOK
    SetErrorLevel 2
    Abort

  owned_install:
    Delete "$INSTDIR\SpatialPlexomera.exe"
    RMDir /r "$INSTDIR\engine"

  fresh_install:
  SetOutPath "$INSTDIR"
  File "${SOURCE_DIR}\SpatialPlexomera.exe"
  File /r "${SOURCE_DIR}\engine"

  WriteUninstaller "$INSTDIR\Uninstall SpatialPlexomera.exe"

  FileOpen $R0 "$INSTDIR\${INSTALL_MARKER}" w
  FileWrite $R0 "SpatialPlexomera ${APP_VERSION}$\r$\n"
  FileClose $R0
  ...
  ${Else}
    ...
    CreateDirectory "$SMPROGRAMS\SpatialPlexomera"
    CreateShortcut "$SMPROGRAMS\SpatialPlexomera\SpatialPlexomera.lnk" "$INSTDIR\SpatialPlexomera.exe" "" "$INSTDIR\SpatialPlexomera.exe"
    CreateShortcut "$SMPROGRAMS\SpatialPlexomera\Uninstall SpatialPlexomera.lnk" "$INSTDIR\Uninstall SpatialPlexomera.exe"
    CreateShortcut "$DESKTOP\SpatialPlexomera.lnk" "$INSTDIR\SpatialPlexomera.exe" "" "$INSTDIR\SpatialPlexomera.exe"

    WriteRegStr HKCU "${APP_REGISTRY_KEY}" "DisplayName" "SpatialPlexomera"
    ...
    WriteRegStr HKCU "${APP_REGISTRY_KEY}" "DisplayIcon" "$INSTDIR\SpatialPlexomera.exe"
    WriteRegStr HKCU "${APP_REGISTRY_KEY}" "UninstallString" "$\"$INSTDIR\Uninstall SpatialPlexomera.exe$\""
  ...
  ${If} $UpdateMode == "1"
    ...
    ${EndIf}
    ${If} $SmokeMode == "1"
      Exec '"$INSTDIR\SpatialPlexomera.exe" ${QA_SMOKE_ARGUMENT}'
    ${Else}
      Exec '"$INSTDIR\SpatialPlexomera.exe"'
    ${EndIf}
  ${EndIf}
SectionEnd
...
Section "Uninstall"
  SetShellVarContext current
  ${If} $SmokeMode != "1"
    Delete "$DESKTOP\SpatialPlexomera.lnk"
    Delete "$SMPROGRAMS\SpatialPlexomera\SpatialPlexomera.lnk"
    Delete "$SMPROGRAMS\SpatialPlexomera\Uninstall SpatialPlexomera.lnk"
    RMDir "$SMPROGRAMS\SpatialPlexomera"
  ...
  Delete "$INSTDIR\Uninstall SpatialPlexomera.exe"
  RMDir "$INSTDIR"
SectionEnd
