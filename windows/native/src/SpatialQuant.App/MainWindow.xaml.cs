...
using SpatialQuant.Windows.Controls;
using SpatialQuant.Windows.Models;
using SpatialQuant.Windows.Services;
...
namespace SpatialQuant.Windows;
...
public partial class MainWindow : Window
{
    ...
    {
        var restoreFolder = Environment.GetEnvironmentVariable("SPATIALQUANT_QA_RESTORE_OUTPUT_FOLDER");
    ...
    {
        var capturePath = Environment.GetEnvironmentVariable("SPATIALQUANT_CAPTURE_PATH");
        if (string.IsNullOrWhiteSpace(capturePath)) return;

        var qaInput = Environment.GetEnvironmentVariable("SPATIALQUANT_QA_INPUT_FOLDER");
        var qaOutput = Environment.GetEnvironmentVariable("SPATIALQUANT_QA_OUTPUT_FOLDER");
        ...
        var qaSection = Environment.GetEnvironmentVariable("SPATIALQUANT_QA_SECTION")?.Trim();
        ...
        if (DetailHost.Content is ScrollViewer detailScroll
            && double.TryParse(
                Environment.GetEnvironmentVariable("SPATIALQUANT_QA_SCROLL_OFFSET"),
                NumberStyles.Float,
        ...
        if (string.Equals(
                Environment.GetEnvironmentVariable("SPATIALQUANT_CAPTURE_EXIT"),
                "1",
                StringComparison.Ordinal))
        {
            if (int.TryParse(
                    Environment.GetEnvironmentVariable("SPATIALQUANT_QA_EXIT_DELAY_MS"),
    ...
    private void ApplyLocalizedFontFamily()
    {
        ...
        // English-only SpatialQuant brand retain their explicit Segoe families.
    ...
    private static void ValidateRestoredHistory(JsonElement response)
    {
        ...
        {
            throw new InvalidDataException("The saved SpatialQuant history has an invalid structure.");
        }
