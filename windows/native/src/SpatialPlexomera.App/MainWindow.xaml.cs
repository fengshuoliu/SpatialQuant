...
using SpatialPlexomera.Windows.Controls;
using SpatialPlexomera.Windows.Models;
using SpatialPlexomera.Windows.Services;
...
namespace SpatialPlexomera.Windows;
...
public partial class MainWindow : Window
{
    ...
    {
        var restoreFolder = Environment.GetEnvironmentVariable("SPATIALPLEXOMERA_QA_RESTORE_OUTPUT_FOLDER");
    ...
    {
        var capturePath = Environment.GetEnvironmentVariable("SPATIALPLEXOMERA_CAPTURE_PATH");
        if (string.IsNullOrWhiteSpace(capturePath)) return;

        var qaInput = Environment.GetEnvironmentVariable("SPATIALPLEXOMERA_QA_INPUT_FOLDER");
        var qaOutput = Environment.GetEnvironmentVariable("SPATIALPLEXOMERA_QA_OUTPUT_FOLDER");
        ...
        var qaSection = Environment.GetEnvironmentVariable("SPATIALPLEXOMERA_QA_SECTION")?.Trim();
        ...
        if (DetailHost.Content is ScrollViewer detailScroll
            && double.TryParse(
                Environment.GetEnvironmentVariable("SPATIALPLEXOMERA_QA_SCROLL_OFFSET"),
                NumberStyles.Float,
        ...
        if (string.Equals(
                Environment.GetEnvironmentVariable("SPATIALPLEXOMERA_CAPTURE_EXIT"),
                "1",
                StringComparison.Ordinal))
        {
            if (int.TryParse(
                    Environment.GetEnvironmentVariable("SPATIALPLEXOMERA_QA_EXIT_DELAY_MS"),
    ...
    private void ApplyLocalizedFontFamily()
    {
        ...
        // English-only SpatialPlexomera brand retain their explicit Segoe families.
    ...
    private static void ValidateRestoredHistory(JsonElement response)
    {
        ...
        {
            throw new InvalidDataException("The saved SpatialPlexomera history has an invalid structure.");
        }
