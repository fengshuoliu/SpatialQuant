    ...
    static func zipData(entries: [Entry], errorContext: String = "ZIP") throws -> Data {
        ...
        for entry in entries {
            guard let fileNameData = entry.fileName.data(using: .utf8),
                ...
                throw SpatialQuantError.message("Could not write \(errorContext) because an archive entry is too large.")
            }
        ...
        for (index, entry) in entries.enumerated() {
            guard let fileNameData = entry.fileName.data(using: .utf8) else {
                throw SpatialQuantError.message("Could not write \(errorContext) because an archive file name is invalid.")
            }
            let crc = crc32(entry.data)
        ...
        guard entries.count <= UInt16.max,
            ...
            throw SpatialQuantError.message("Could not write \(errorContext) because the archive directory is too large.")
        }
