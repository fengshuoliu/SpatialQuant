...
struct SpatialQuantCanvasBackground: View {
    @Environment(\.colorScheme) private var colorScheme

    var body: some View {
        ZStack {
            SpatialQuantDesign.canvas

            LinearGradient(
                stops: [
                    .init(color: SpatialQuantDesign.spectralCyan.opacity(spectralOpacity), location: 0),
                    .init(color: Color.clear, location: 0.34),
                    .init(color: SpatialQuantDesign.spectralMagenta.opacity(spectralOpacity * 0.52), location: 0.69),
                    .init(color: SpatialQuantDesign.spectralGreen.opacity(spectralOpacity * 0.76), location: 1)
                ],
...
extension View {
    func spatialQuantWindowMaterial() -> some View {
        modifier(SpatialQuantWindowMaterialModifier())
    }

    func spatialQuantGlassSurface(
        cornerRadius: CGFloat = SpatialQuantDesign.panelRadius,
        tint: Color? = nil,
        interactive: Bool = false
    ) -> some View {
        modifier(
            SpatialQuantGlassSurfaceModifier(
    ...
    func spatialQuantGlassCapsule(tint: Color? = nil) -> some View {
        modifier(SpatialQuantGlassCapsuleModifier(tint: tint))
    }

    func spatialQuantSelectedGlass(isSelected: Bool, tint: Color) -> some View {
        modifier(SpatialQuantSelectedGlassModifier(isSelected: isSelected, tint: tint))
    }

    func spatialQuantProminentButtonStyle() -> some View {
        modifier(SpatialQuantProminentButtonModifier())
    }
}

private struct SpatialQuantWindowMaterialModifier: ViewModifier {
    @ViewBuilder
...
private struct SpatialQuantGlassSurfaceModifier: ViewModifier {
    ...
    func body(content: Content) -> some View {
        ...
        if reduceTransparency {
            content
                .background(SpatialQuantDesign.panel, in: shape)
                .overlay {
                    shape.stroke(SpatialQuantDesign.panelBorder, lineWidth: 1)
                }
                .shadow(color: SpatialQuantDesign.glassAmbientShadow, radius: 10, y: 4)
        } else if #available(macOS 26.0, *) {
            if interactive {
                content
                    .glassEffect(.regular.tint(tint ?? SpatialQuantDesign.glassNeutralTint).interactive(), in: shape)
                    .overlay {
                        SpatialQuantOpticalEdge(shape: shape, tint: tint, intensity: isHovering ? 1.12 : 0.92)
                    }
                    .shadow(color: SpatialQuantDesign.glassShadow, radius: isHovering ? 18 : 14, y: isHovering ? 7 : 5)
                    .shadow(color: SpatialQuantDesign.glassAmbientShadow, radius: 4, y: -1)
                ...
                content
                    .glassEffect(.regular.tint(tint ?? SpatialQuantDesign.glassNeutralTint), in: shape)
                    .overlay {
                        SpatialQuantOpticalEdge(shape: shape, tint: tint, intensity: 0.82)
                    }
                    .shadow(color: SpatialQuantDesign.glassShadow, radius: 15, y: 6)
                    .shadow(color: SpatialQuantDesign.glassAmbientShadow, radius: 4, y: -1)
            }
        } else {
            content
...
private struct SpatialQuantGlassCapsuleModifier: ViewModifier {
    var tint: Color?
    @Environment(\.accessibilityReduceTransparency) private var reduceTransparency
    ...
    func body(content: Content) -> some View {
        if reduceTransparency {
            content
                .background(SpatialQuantDesign.panel, in: Capsule())
                .overlay {
                    Capsule().stroke(SpatialQuantDesign.panelBorder, lineWidth: 1)
                }
        } else if #available(macOS 26.0, *) {
            content
                .glassEffect(.regular.tint(tint ?? SpatialQuantDesign.glassNeutralTint), in: Capsule())
                .overlay {
                    SpatialQuantOpticalEdge(shape: Capsule(), tint: tint, intensity: 0.88)
                }
                .shadow(color: SpatialQuantDesign.glassAmbientShadow, radius: 7, y: 3)
        } else {
            content
...
private struct SpatialQuantSelectedGlassModifier: ViewModifier {
    var isSelected: Bool
    var tint: Color
    ...
    func body(content: Content) -> some View {
        if #available(macOS 26.0, *), isSelected {
            ...
            content
                ...
                .overlay {
                    SpatialQuantOpticalEdge(shape: shape, tint: tint, intensity: 1.1)
                }
...
private struct SpatialQuantProminentButtonModifier: ViewModifier {
    @ViewBuilder
    func body(content: Content) -> some View {
...
private struct SpatialQuantOpticalEdge<S: InsettableShape>: View {
    var shape: S
    var tint: Color?
            ...
            shape.stroke(
                LinearGradient(
                    stops: [
                        ...
                        .init(color: SpatialQuantDesign.spectralCyan.opacity(0.40 * intensity), location: 0.18),
                        .init(color: Color.white.opacity(0.14 * intensity), location: 0.45),
                        .init(color: semanticTint.opacity(0.34 * intensity), location: 0.67),
                        .init(color: SpatialQuantDesign.spectralMagenta.opacity(0.22 * intensity), location: 0.82),
...
struct WorkflowStatusBadge: View {
    ...
    var body: some View {
        Text(LocalizedStringKey(status.title))
            ...
            .padding(.vertical, 5)
            .spatialQuantGlassCapsule(tint: status.foregroundColor.opacity(0.16))
