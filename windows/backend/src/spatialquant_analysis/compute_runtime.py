"""Optional, deterministic CPU + OpenCL compute runtime for SpatialQuant.
...
class _OpenCLWorker:
    ...
    def __init__(self, descriptor: dict[str, Any], device: Any):
        ...
        self._executor = ThreadPoolExecutor(
            max_workers=1,
            thread_name_prefix=f"spatialquant-{descriptor['id'].replace(':', '-')}",
        )
...
class _CpuWorker:
    ...
    def __init__(self, lane_id: int):
        ...
        self._executor = ThreadPoolExecutor(
            max_workers=1,
            thread_name_prefix=f"spatialquant-cpu-{self.lane_id}",
        )
        self._closed = False
