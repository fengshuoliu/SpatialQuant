...
struct SpatialQuantApp: App {
    ...
    var body: some Scene {
        WindowGroup("SpatialQuant", id: "main") {
            ContentView()
        ...
        MenuBarExtra {
            SpatialQuantMenuBarView(
                updater: updaterController.updater,
                language: store.uiLanguage
            )
        } label: {
            SpatialQuantMenuBarIcon()
        }
...
private struct SpatialQuantMenuBarIcon: View {
    ...
    var body: some View {
        Image(nsImage: image)
            .accessibilityLabel("SpatialQuant")
    }
}

private struct SpatialQuantMenuBarView: View {
    ...
    var body: some View {
        Button(language.localized("Show SpatialQuant")) {
            showMainWindow()
        }
        ...
        Button(language.localized("Quit SpatialQuant")) {
            NSApp.terminate(nil)
        }
    }
    ...
    private func showMainWindow() {
        NSApp.setActivationPolicy(.regular)
        if let window = NSApp.windows.first(where: { $0.title == "SpatialQuant" && $0.canBecomeMain }) {
