...
public partial class MainWindow
{
    ...
    private async Task CheckForUpdatesAsync(bool isManual)
    {
        ...
        {
            Debug.WriteLine($"SpatialPlexomera update check failed: {exception}");
    ...
    private async Task OfferAndInstallUpdateAsync(WindowsUpdateRelease release)
    {
        ...
        {
            Debug.WriteLine($"SpatialPlexomera update installation failed: {exception}");
            if (!_closeInProgress)
            {
    ...
    private void TryRecordSuccessfulUpdateCheck()
    {
        ...
        {
            Debug.WriteLine($"SpatialPlexomera could not save update-check state: {exception.Message}");
        }
    ...
    {
        var value = Environment.GetEnvironmentVariable("SPATIALPLEXOMERA_DISABLE_UPDATE_CHECK");
    ...
    private static bool IsInstallerOwnedCurrentCopy()
    {
        try
        {
            ...
            if (string.IsNullOrWhiteSpace(executableDirectory)
                || !File.Exists(Path.Combine(executableDirectory, ".spatialplexomera-install")))
                return false;

            using var key = Registry.CurrentUser.OpenSubKey(
                @"Software\Microsoft\Windows\CurrentVersion\Uninstall\SpatialPlexomera",
                writable: false);
