...
{
    public const string InstallerAssetName = "SpatialQuant-Windows-x64-Setup.exe";
    public const string ChecksumAssetName = "SHA256SUMS-Windows.txt";
    public static readonly Uri ReleasesApiUri = new(
        "https://api.github.com/repos/fengshuoliu/SpatialQuant/releases?per_page=100");
    ...
    public async Task<WindowsUpdateRelease?> CheckForUpdateAsync(CancellationToken cancellationToken = default)
    {
        ...
        request.Headers.UserAgent.ParseAdd($"SpatialQuant-Windows/{FormatVersion(_currentVersion)}");
        ...
        out WindowsUpdateRelease candidate)
    {
        ...
        var releasePage = new Uri(
            $"https://github.com/fengshuoliu/SpatialQuant/releases/tag/{Uri.EscapeDataString(tagName)}");
    ...
    {
        var expectedPath = $"/fengshuoliu/SpatialQuant/releases/download/{Uri.EscapeDataString(tagName)}/{Uri.EscapeDataString(assetName)}";
