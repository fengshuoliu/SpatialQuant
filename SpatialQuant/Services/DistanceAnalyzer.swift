...
enum DistanceAnalyzer {
    static func runNearestNeighborAnalysis(
        ...
        guard !assignments.isEmpty else {
            throw SpatialQuantError.message("Run cell-type assignment before distance analysis.")
        }
        ...
        guard !targetType.isEmpty else {
            throw SpatialQuantError.message("Select a target cell type.")
        }
        guard !queryTypes.isEmpty else {
            throw SpatialQuantError.message("Select at least one query cell type.")
        }
    ...
    static func runBoundaryDistanceAnalysis(
        ...
        guard boundaryMask.width == max(1, canvasWidth),
            ...
            throw SpatialQuantError.message("Selected boundary mask does not match the current cell-type image size.")
        }
    ...
    private static func runBoundaryDistanceAnalysis(
        ...
        guard !boundaryMask.isEmpty else {
            throw SpatialQuantError.message("Selected boundary mask is empty.")
        }
        let queryTypes = orderedNonEmpty(queryTypes)
        guard !queryTypes.isEmpty else {
            throw SpatialQuantError.message("Select at least one query cell type.")
        ...
        guard !seed.isEmpty else {
            throw SpatialQuantError.message("Selected boundary mask does not contain a valid drawable edge.")
        }
        ...
        guard !rows.isEmpty else {
            throw SpatialQuantError.message("No cells found after applying the selected boundary filter.")
        }
    ...
    private static func nearestDistances(
        ...
        guard !targets.isEmpty else {
            throw SpatialQuantError.message("No cells found for target cell type \(targetType).")
        }
        guard !queries.isEmpty else {
            throw SpatialQuantError.message("No cells found for query cell type \(queryType).")
        }
        if targetType == queryType, queries.count < 2 {
            throw SpatialQuantError.message("Need at least 2 cells of type \(targetType) for self-type nearest neighbor.")
        }
    ...
    private static func loadBoundaryMask(from url: URL) throws -> RasterMask {
        guard let image = NSImage(contentsOf: url),
            ...
            throw SpatialQuantError.message("Could not read selected boundary mask.")
        }
