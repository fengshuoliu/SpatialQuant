...
final class AppStore: ObservableObject {
    ...
    private let logger = Logger(subsystem: "com.fengshuoliu.SpatialPlexomera", category: "Pipeline")
    private let defaults = UserDefaults.standard
    private let inputFolderKey = "SpatialPlexomera.inputFolder"
    private let outputFolderKey = "SpatialPlexomera.outputFolder"
    private let cpuAllocationKey = "SpatialPlexomera.cpuAllocationPercent"
    private let gpuAllocationKey = "SpatialPlexomera.gpuAllocationPercent"
    private let maximumCPUDefaultMigrationKey = "SpatialPlexomera4.maximumCPUDefaultApplied"
    private let nucleiScanCombinationBudgetKey = "SpatialPlexomera.nucleiScanCombinationBudget"
    private let nucleiScanSecondsPerCombinationKey = "SpatialPlexomera.nucleiScanSecondsPerCombination"
    private let nucleiScanBenchmarkCPUAllocationKey = "SpatialPlexomera.nucleiScanBenchmarkCPUAllocation"
    ...
    private static func defaultWorkspaceFolder(named name: String) -> URL {
        ...
        return documents
            .appendingPathComponent("SpatialPlexomera", isDirectory: true)
        ...
        currentTask = Task.detached(priority: .userInitiated) { [store = self] in
            do {
                ...
                guard !adjustedRegionIDs.isEmpty else {
                    throw SpatialPlexomeraError.message("No adjusted ROI was generated from the selected cells.")
                }
        ...
        currentTask = Task.detached(priority: .userInitiated) { [store = self] in
            do {
                ...
                guard let displayResult = OutputWriter.loadCellDistributionResult(outputFolder: outputFolder) else {
                    throw SpatialPlexomeraError.message("Cell Distribution export finished but no readable result was found.")
                }
                await MainActor.run {
