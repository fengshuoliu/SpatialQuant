...
public partial class MainWindow
{
    ...
    private async Task CheckForUpdatesAsync(bool isManual)
    {
        ...
        {
            Debug.WriteLine($"SpatialQuant update check failed: {exception}");
    ...
    private async Task OfferAndInstallUpdateAsync(WindowsUpdateRelease release)
    {
        ...
        {
            Debug.WriteLine($"SpatialQuant update installation failed: {exception}");
            if (!_closeInProgress)
            {
    ...
    private void TryRecordSuccessfulUpdateCheck()
    {
        ...
        {
            Debug.WriteLine($"SpatialQuant could not save update-check state: {exception.Message}");
        }
    ...
    {
        var value = Environment.GetEnvironmentVariable("SPATIALQUANT_DISABLE_UPDATE_CHECK");
    ...
    private static bool IsInstallerOwnedCurrentCopy()
    {
        try
        {
            ...
            if (string.IsNullOrWhiteSpace(executableDirectory)
                || !File.Exists(Path.Combine(executableDirectory, ".spatialquant-install")))
                return false;

            using var key = Registry.CurrentUser.OpenSubKey(
                @"Software\Microsoft\Windows\CurrentVersion\Uninstall\SpatialQuant",
                writable: false);
