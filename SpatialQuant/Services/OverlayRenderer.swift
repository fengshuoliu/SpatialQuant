...
enum OverlayRenderer {
    ...
    static func render(
        ...
        guard let firstChannel = overlayChannels.first,
            ...
            throw SpatialQuantError.message("No CSV channels are selected for overlay rendering.")
        }

        for channel in overlayChannels {
            guard let matrix = matrixByFile[channel.fileName] else {
                throw SpatialQuantError.message("Missing matrix for \(channel.fileName).")
            }
            if matrix.width != firstMatrix.width || matrix.height != firstMatrix.height {
                throw SpatialQuantError.message("\(channel.fileName) has shape \(matrix.width)x\(matrix.height), expected \(firstMatrix.width)x\(firstMatrix.height).")
            }
        }
    ...
    private static func renderSplitPanels(
        ...
        guard !channelMatrices.isEmpty else {
            throw SpatialQuantError.message("No matrices are loaded.")
        }
