...
enum NPZExportService {
    ...
    static func readInt16Array(named name: String, from url: URL) throws -> Int16Array2D {
        ...
        guard let npy = entries[entryName] else {
            throw SpatialQuantError.message("Could not read \(name) from \(url.lastPathComponent).")
        }
        let array = try readNPY(npy, expectedName: name)
        guard array.descr == "<i2" || array.descr == "|i2" else {
            throw SpatialQuantError.message("Could not read \(name) because dtype \(array.descr) is not int16.")
        }
        let expectedBytes = array.width * array.height * MemoryLayout<Int16>.stride
        guard array.data.count >= expectedBytes else {
            throw SpatialQuantError.message("Could not read \(name) because the NumPy payload is truncated.")
        }
        var values: [Int16] = []
    ...
    private static func validateDimensions(width: Int, height: Int, count: Int) throws {
        guard width > 0, height > 0, width * height == count else {
            throw SpatialQuantError.message("Could not write NPZ because array dimensions do not match the data.")
        }
    }
    ...
    private static func npyData(for array: Array2D) throws -> Data {
        ...
        guard header.utf8.count <= UInt16.max else {
            throw SpatialQuantError.message("Could not write NPZ because a NumPy header is too large.")
        }
    ...
    private static func zipData(entries: [ZipEntry]) throws -> Data {
        ...
        for entry in entries {
            guard let fileNameData = entry.fileName.data(using: .utf8),
                ...
                throw SpatialQuantError.message("Could not write NPZ because a ZIP entry is too large.")
            }
        ...
        for (index, entry) in entries.enumerated() {
            guard let fileNameData = entry.fileName.data(using: .utf8) else {
                throw SpatialQuantError.message("Could not write NPZ because a ZIP file name is invalid.")
            }
        ...
        guard entries.count <= UInt16.max,
            ...
            throw SpatialQuantError.message("Could not write NPZ because the ZIP directory is too large.")
        }
    ...
    private static func deflate(_ data: Data) throws -> Data {
        ...
        throw SpatialQuantError.message("Could not compress NPZ data.")
    }
    ...
    private static func readZipEntries(from data: Data) throws -> [String: Data] {
        ...
        while offset + 30 <= data.count {
            ...
            guard signature == 0x04034b50 else {
                throw SpatialQuantError.message("Could not read NPZ because the ZIP local header is invalid.")
            }
            ...
            guard flags & 0x0008 == 0 else {
                throw SpatialQuantError.message("Could not read NPZ because ZIP data descriptors are not supported.")
            }
            ...
            guard extraStart <= data.count, payloadStart <= data.count else {
                throw SpatialQuantError.message("Could not read NPZ because a ZIP entry is truncated.")
            }
            let nameData = Data(data[nameStart..<(nameStart + fileNameLength)])
            guard let name = String(data: nameData, encoding: .utf8) else {
                throw SpatialQuantError.message("Could not read NPZ because a ZIP entry name is invalid.")
            }
            ...
            guard let compressedSize = compressedSize64.flatMap({ $0 <= UInt64(Int.max) ? Int($0) : nil }),
                ...
                throw SpatialQuantError.message("Could not read NPZ because a ZIP64 entry is too large.")
            }
            ...
            default:
                throw SpatialQuantError.message("Could not read NPZ because ZIP compression method \(method) is unsupported.")
            }
    ...
    private static func inflateRawDeflate(_ data: Data, uncompressedSize: Int) throws -> Data {
        ...
        guard decoded > 0 else {
            throw SpatialQuantError.message("Could not inflate compressed NPZ data.")
        }
    ...
    private static func readNPY(_ data: Data, expectedName: String) throws -> Array2D {
        guard data.count >= 10,
            ...
            throw SpatialQuantError.message("Could not read \(expectedName) because the NumPy header is invalid.")
        }
        let major = data[6]
        let headerStart: Int
        ...
        if major == 1 {
            ...
            throw SpatialQuantError.message("Could not read \(expectedName) because NumPy format \(major) is unsupported.")
        }
        let headerEnd = headerStart + headerLength
        guard headerEnd <= data.count else {
            throw SpatialQuantError.message("Could not read \(expectedName) because the NumPy header is truncated.")
        }
        guard let header = String(data: Data(data[headerStart..<headerEnd]), encoding: .ascii) else {
            throw SpatialQuantError.message("Could not read \(expectedName) because the NumPy header is not ASCII.")
        }
        let descr = npyHeaderValue("descr", in: header)
        guard let shape = npyShape(in: header), shape.count == 2 else {
            throw SpatialQuantError.message("Could not read \(expectedName) because only 2D arrays are supported.")
        }
        let height = shape[0]
        let width = shape[1]
        guard width > 0, height > 0 else {
            throw SpatialQuantError.message("Could not read \(expectedName) because array dimensions are invalid.")
        }
        return Array2D(
            name: expectedName,
            descr: descr ?? "",
