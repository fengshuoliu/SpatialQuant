...
enum ImageExportService {
    ...
    static func makeCGImage(width: Int, height: Int, rgba: [UInt8]) throws -> CGImage {
        ...
        guard let provider = CGDataProvider(data: data as CFData) else {
            throw SpatialQuantError.message("Could not create image data provider.")
        }
        ...
        guard let image = CGImage(
            ...
            throw SpatialQuantError.message("Could not create CGImage.")
        }
        return image
    }
    ...
    static func pngData(from image: NSImage, dpi: Int = 300) throws -> Data {
        guard let tiff = image.tiffRepresentation,
            ...
            throw SpatialQuantError.message("Could not encode PNG.")
        }
        bitmap.size = NSSize(
    ...
    static func writeTIFF(_ image: NSImage, to url: URL) throws {
        ...
        guard let data = image.tiffRepresentation else {
            throw SpatialQuantError.message("Could not encode TIFF.")
        }
    ...
    static func writeUInt8TIFF(width: Int, height: Int, values: [UInt8], to url: URL) throws {
        ...
        guard values.count == width * height else {
            throw SpatialQuantError.message("Could not encode TIFF because the mask dimensions do not match the data.")
        }
        try FileManager.default.createDirectory(at: url.deletingLastPathComponent(), withIntermediateDirectories: true)
        ...
        guard CGImageDestinationFinalize(destination) else {
            throw SpatialQuantError.message("Could not finalize TIFF.")
        }
    }
    ...
    static func writeUInt16RasterRaw(width: Int, height: Int, values: [UInt16], to url: URL) throws {
        ...
        let height = max(1, height)
        guard values.count == width * height else {
            throw SpatialQuantError.message("Could not encode raw mask because the dimensions do not match the data.")
        }
        try FileManager.default.createDirectory(at: url.deletingLastPathComponent(), withIntermediateDirectories: true)
    ...
    static func writeAI(_ image: NSImage, to url: URL, dpi: Int = 300) throws {
        ...
        guard let consumer = CGDataConsumer(url: url as CFURL),
            ...
            throw SpatialQuantError.message("Could not encode AI-compatible PDF.")
        }
    ...
    static func lightenedRGBA(from image: NSImage, width: Int, height: Int, blend: Double = 0.45) throws -> [UInt8] {
        guard let source = cgImage(from: image) else {
            throw SpatialQuantError.message("Could not read overlay image pixels.")
        }
        let width = max(1, width)
        let height = max(1, height)
        ...
        try rgba.withUnsafeMutableBytes { buffer in
            guard let context = CGContext(
                ...
                throw SpatialQuantError.message("Could not create overlay bitmap context.")
            }
    ...
    private static func rasterPixels(from image: NSImage) throws -> (width: Int, height: Int, pixels: [RGBPixel], background: RGBPixel) {
        guard let source = cgImage(from: image) else {
            throw SpatialQuantError.message("Could not read image pixels for vector export.")
        }
        ...
        try rgba.withUnsafeMutableBytes { buffer in
            guard let context = CGContext(
                ...
                throw SpatialQuantError.message("Could not create vector export bitmap context.")
            }
