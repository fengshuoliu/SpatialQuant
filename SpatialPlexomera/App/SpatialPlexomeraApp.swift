...
@main
struct SpatialPlexomeraApp: App {
    ...
    var body: some Scene {
        WindowGroup("SpatialPlexomera", id: "main") {
            ContentView()
        ...
        MenuBarExtra {
            SpatialPlexomeraMenuBarView(
                updater: updaterController.updater,
                language: store.uiLanguage
            )
        } label: {
            SpatialPlexomeraMenuBarIcon()
        }
...
private struct SpatialPlexomeraMenuBarIcon: View {
    ...
    var body: some View {
        Image(nsImage: image)
            .accessibilityLabel("SpatialPlexomera")
    }
}

private struct SpatialPlexomeraMenuBarView: View {
    ...
    var body: some View {
        Button(language.localized("Show SpatialPlexomera")) {
            showMainWindow()
        }
        ...
        Button(language.localized("Quit SpatialPlexomera")) {
            NSApp.terminate(nil)
        }
    }
    ...
    private func showMainWindow() {
        NSApp.setActivationPolicy(.regular)
        if let window = NSApp.windows.first(where: { $0.title == "SpatialPlexomera" && $0.canBecomeMain }) {
