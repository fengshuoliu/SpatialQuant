...
DEFAULT_NATIVE_THREADS = max(1, int(os.environ.get("SPATIALPLEXOMERA_NATIVE_THREADS", str(max(1, CPU_COUNT - 1)))))
DEFAULT_SWEEP_WORKERS = max(1, min(CPU_COUNT, int(os.environ.get("SPATIALPLEXOMERA_SWEEP_WORKERS", str(max(1, CPU_COUNT - 1))))))
...
APP_ICON_PATH = APP_ROOT / "assets" / "SpatialPlexomera.png"
ZH_HANS_STRINGS_PATH = APP_ROOT / "assets" / "zh-Hans.strings"
st.set_page_config(
    page_title="SpatialPlexomera",
    page_icon=str(APP_ICON_PATH) if APP_ICON_PATH.exists() else None,
    layout="wide",
    initial_sidebar_state="expanded",
)
TMP_ROOT = Path(os.environ.get("SPATIALPLEXOMERA_SESSION_ROOT", tempfile.gettempdir())) / "SpatialPlexomera" / "sessions"
SETTINGS_PATH = Path(
    os.environ.get(
        "SPATIALPLEXOMERA_SETTINGS_PATH",
        str(Path.home() / ".spatialplexomera" / "settings.json"),
    )
)
DESKTOP_PATHS_PATH = Path(
    os.environ.get(
        "SPATIALPLEXOMERA_DESKTOP_PATHS_PATH",
...
def _load_ui_language_setting() -> str:
    requested = os.environ.get("SPATIALPLEXOMERA_UI_LANGUAGE", "system").strip().lower()
    try:
...
def _effective_ui_language() -> str:
    ...
    if requested == "system":
        requested = os.environ.get("SPATIALPLEXOMERA_SYSTEM_LANGUAGE", "en").strip().lower()
    ...
    st.iframe(
        ...
        (() => {{
          try {{
            ...
            if (hostWindow.__spatialPlexomeraTranslationObserver) {{
              hostWindow.__spatialPlexomeraTranslationObserver.disconnect();
            }}
            apply(doc.body);
            ...
            hostWindow.__spatialPlexomeraTranslationObserver = observer;
          }} catch (error) {{
            console.warn("SpatialPlexomera UI translation could not be applied", error);
          }}
        }})();
        </script>
        """,
...
try:
    ...
    from src.spatialplexomera_analysis.celltype_assignment import (
    ...
    from src.spatialplexomera_analysis.distance_analysis import (
        discover_boundary_masks,
        run_boundary_distance_analysis,
        run_nearest_neighbor_analysis,
    )
    from src.spatialplexomera_analysis.io import (
    ...
    from src.spatialplexomera_analysis.models import ChannelConfig, NucleiParams, PipelineConfig, RegionParams
    from src.spatialplexomera_analysis.nuclei_segmentation import (
        SWEEP_PARAM_LABELS,
        SWEEP_PARAM_ORDER,
    ...
    from src.spatialplexomera_analysis.region_analysis import (
    ...
    from src.spatialplexomera_analysis.neighborhood_analysis import (
        make_neighborhood_figure,
        run_neighborhood_analysis,
        save_neighborhood_analysis_outputs,
    )
    from src.spatialplexomera_analysis.visualization import COMMON_FIRST, overlay_multi_channels, plot_split_channels
...
if RUNTIME_IMPORT_ERROR is not None:
    st.title("SpatialPlexomera analysis pipeline")
    st.error(
        "SpatialPlexomera could not start its bundled scientific runtime. "
...
def current_output_zip_path(output_signature: str | None = None) -> Path:
    ...
    return session_workspace_root() / f"SpatialPlexomera_outputs_{st.session_state['session_id']}_{short_hash}.zip"
...
def _render_integrated_region_selection_ui(
    *,
    ...
    if selected_mask_labels and selected_celltypes and selected_masks:
        from src.spatialplexomera_analysis.region_analysis import make_roi_comparison_figure
...
def build_and_save_config(available_files: Sequence[str], uploaded_files) -> None:
    ...
    if input_mode == "Upload files":
        ...
        save_dir = resolve_folder(output_value) if output_value else folder / "SpatialPlexomera_outputs"
        save_dir.mkdir(parents=True, exist_ok=True)
...
def render_sidebar_navigation() -> str:
    ...
    with st.sidebar:
        ...
        with st.expander("Session info", expanded=False):
            ...
            st.info(
                "SpatialPlexomera can read a local folder or uploaded ImageJ-exported CSV/TXT files. "
    ...
    with tab:
        ...
        if st.session_state.get("input_mode_radio") == "Local folder":
            st.text_input(
                ...
                placeholder=r"C:\Data\SpatialPlexomera\channels",
            )
            st.text_input(
                "Output folder",
                key="local_output_folder",
                placeholder=r"C:\Data\SpatialPlexomera\results",
            )
    ...
    with tab:
        ...
        if zip_bytes:
            st.download_button(
                ...
                data=zip_bytes,
                file_name=f"SpatialPlexomera_outputs_{st.session_state['session_id']}.zip",
    ...
    try:
        ...
        st.markdown(
            ...
            <div style="display:flex;align-items:center;gap:14px;margin-bottom:0.65rem;position:relative;z-index:1;">
              ...
              <div style="font-size:2rem;font-weight:700;line-height:1.1;letter-spacing:0;">SpatialPlexomera</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
