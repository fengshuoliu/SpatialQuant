...
namespace SpatialPlexomera.Windows.Updates;

public static class UpdateInstallerLauncher
{
    public const string InstanceMutexName = @"Local\SpatialPlexomera.Windows.Application";
    public const string QaInstanceMutexEnvironmentVariable = "SPATIALPLEXOMERA_QA_INSTANCE_MUTEX";
    public const string QaSmokeLaunchArgument = "--qa-smoke";
    public const string QaSmokeMarkerFileName = ".spatialplexomera-smoke";
    private const string QaInstanceMutexPrefix = @"Local\SpatialPlexomera.Windows.Application.QA.";
        ...
        int parentProcessId)
    {
        ...
        return Process.Start(startInfo)
            ?? throw new InvalidOperationException("Windows did not start the SpatialPlexomera update installer.");
    }
    ...
    private static void EnsureChildPath(string candidate, string expectedParent)
    {
        ...
        {
            throw new InvalidOperationException("The installer path escaped the private SpatialPlexomera update directory.");
        }
    }
}
