...
enum RegionAnalyzer {
    ...
    static func run(
        ...
        guard !assignments.isEmpty else {
            throw SpatialPlexomeraError.message("Run cell-type assignment before region analysis.")
        }
        ...
        guard !selectedTypes.isEmpty else {
            throw SpatialPlexomeraError.message("Select at least one assigned cell type for region analysis.")
        }
    ...
    static func applyManualEdit(
        ...
        guard !cleanName.isEmpty else {
            throw SpatialPlexomeraError.message("Enter a name for the adjusted ROI.")
        }
        let validPolygonGroups = polygonGroups.filter { $0.count >= 3 }
        guard !validPolygonGroups.isEmpty else {
            throw SpatialPlexomeraError.message("Close at least one selection area before saving an adjusted ROI.")
        }

        let targetRegion = targetRegionID.flatMap { id in result.regions.first { $0.id == id } }
        if mode != .redraw, targetRegion == nil {
            throw SpatialPlexomeraError.message("Select an existing ROI before using include or exclude editing.")
        }
        ...
        guard !selectedCells.isEmpty else {
            throw SpatialPlexomeraError.message("The drawn selection areas did not select any assigned cells.")
        }
        ...
        guard !adjustedSeedCells.isEmpty else {
            throw SpatialPlexomeraError.message("The adjusted ROI has no seed cells after applying this edit.")
        }
        ...
        guard !adjustedMask.isEmpty else {
            throw SpatialPlexomeraError.message("The adjusted ROI is empty after applying this edit.")
        }
        ...
        guard var adjustedRegion = makeRegion(
            ...
            throw SpatialPlexomeraError.message("The adjusted ROI could not be converted to a saved region.")
        }
    ...
    private static var isManualROIDebugEnabled: Bool {
        ProcessInfo.processInfo.environment["SPATIALPLEXOMERA_DEBUG_MANUAL_ROI"] == "1"
    }
