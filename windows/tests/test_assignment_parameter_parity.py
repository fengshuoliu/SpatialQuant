...
from src.spatialplexomera_analysis.models import PipelineConfig  # noqa: E402
...
class AssignmentParameterParityTests(unittest.TestCase):
    ...
    def test_wpf_threshold_picker_displays_local_and_retains_triangle(self) -> None:
        source = (
            WINDOWS_DIR / "native" / "src" / "SpatialPlexomera.App" / "MainWindow.xaml.cs"
        ).read_text(encoding="utf-8")
    ...
    def test_assignment_optimizer_invalidates_stale_recommendation_before_artifact_replacement(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="spatialplexomera-assignment-rerun-") as temporary:
            root = Path(temporary)
            engine = NativeEngine()
    ...
    def test_assignment_preprocessing_matches_macos_box_filters_and_caps(self) -> None:
        ...
        with patch(
            "src.spatialplexomera_analysis.celltype_assignment._truncated_box_blur",
